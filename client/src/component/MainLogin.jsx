            ...
            <div className="content">
                <div className="content-header">
                    <div className="title">
                        <div className="title-logo">
                            <span>Movie <span className="blue">King</span></span>
                        </div>
                ...
                <form name="login" className="form-content" onSubmit={handleSubmitted}>
                    <div className="f-item">
                        <input type="text" name="username" className="f-input" placeholder="username" onChange={handleChange} value={username} />
                        { submitted && error && username.length < 10 ? 
                            <i className="bx bx-error-circle icon-error-input"></i> 
                        : <></> }
                    </div>
                    <div className="f-item">
                        <input type="password" name="password" className="f-input" placeholder="password" onChange={handleChange} value={password} />
                        { submitted && error && password.length < 10 ? 
                            <i className="bx bx-error-circle icon-error-input"></i> 
                        : <></> }
                    </div>
                    <div className="f-item content-button">
                        <button type="submit" className={open_button() ? 'btn btn-close' : 'btn btn-open' } disabled={open_button()}>Sign in</button>
                        <Link to="/" className="btn-link btn-back mt">Back</Link>
                    </div>
                </form>
            </div>
        </main>
...
export default styled(MainLogin)`
    ...
    .content-error {
        ...
        padding: .2rem .8rem;
        column-gap: 1rem;
        box-shadow: 0 0 2px .3px rgba(255, 34, 85, 0.5);
    }
    ...
    .btn-close {
        color: #ffffff;
        background-color: rgba(48, 87, 225, .6);
        border-color: rgba(48, 87, 225, .6);
    ...
    .btn-open:hover {
        background-color: rgba(48, 87, 225, .7);
    }
    ...
    .btn-back:hover {
        background-color: rgba(255, 34, 85, .7);
    }
`;
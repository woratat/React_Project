...
function MainRegister({ className }) {
    ...
    const handleSubmitted = async (e) => {
        ...
        if (password === confirmPassword) {
            try {
                const res = await axios.post('http://localhost:5050/api/post/account', {
                    username: username,
                    password: password
                }, { timeout: 2000 });
    ...
    const open_button = () => {
        if (validator.isEmpty(username) || validator.isEmpty(password) || validator.isEmpty(confirmPassword)) {
          return true;
        } else {
          return false;
        }
...
export default styled(MainRegister)`
  display: flex;

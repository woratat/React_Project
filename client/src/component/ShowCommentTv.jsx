...
function ShowComment({ className, tv_token, id }) {
    ...
    const getComment = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5050/api/get/commentTv/${tv_token}`,
          {
            timeout: 2000,
          }
        );
  ...
  const updateMessage = async (comment_id) => {
    ...
    if (text) {
        try {
            const res = await axios.put(`http://localhost:5050/api/update/commentTv/${comment_id}`, {  //update
               message: text
            }, {
                timeout: 2000
            });
  ...
  const delete_comment = async (comment_id) => {
    swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                const res = await axios.delete(`http://localhost:5050/api/delete/commentTv/${comment_id}`, {
                    timeout: 2000
                });
